...
void epaper_driver_display::EPD_LoadFullLut() {
    EPD_SetLut(WF_Full_1IN54);
    EPD_SendCommand(0x22);
    EPD_SendData(0xB1);  // Load temperature + waveform
    EPD_SendCommand(0x20);
    read_busy();
}

void epaper_driver_display::EPD_ReloadPartialLut() {
    read_busy();
    EPD_SetLut(WF_PARTIAL_1IN54_0);
    EPD_SendCommand(0x37); 
    EPD_SendData(0x00); EPD_SendData(0x00); EPD_SendData(0x00); EPD_SendData(0x00);
    EPD_SendData(0x00); EPD_SendData(0x40); EPD_SendData(0x00); EPD_SendData(0x00);
...
void epaper_driver_display::EPD_WriteFrameToRAMSilent(const uint8_t *frame) {
    int len = lcd_spi_data.buffer_len;
    // Write to old-image RAM (reference for partial comparison)
    EPD_SendCommand(0x26);
    writeBytes(frame, len);
    // Write to new-image RAM (current content)
    EPD_SendCommand(0x24);
    writeBytes(frame, len);
    // No display trigger — hardware RAM now matches the physical screen
}

void epaper_driver_display::EPD_GetBuffer(uint8_t *dst) const {
    memcpy(dst, buffer, lcd_spi_data.buffer_len);
}
