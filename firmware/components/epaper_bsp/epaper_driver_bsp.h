...
public:
    ...
    void EPD_LoadFullLut();      // reload full LUT without hardware reset
    void EPD_ReloadPartialLut(); // reload partial LUT without hardware reset

    // Write frame to both EPD RAM banks silently (no display trigger)
    // Call after EPD_Init() to restore last screen — enables partial-only boot update
    void EPD_WriteFrameToRAMSilent(const uint8_t *frame);
    // Copy internal pixel buffer out (5000 bytes for 200×200)
    void EPD_GetBuffer(uint8_t *dst) const;
};
#endif
